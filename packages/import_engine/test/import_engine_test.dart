...
void main() {
  ...
  test(
    'same code in different places stays as separate structure occurrences',
    () {
      ...
      expect(
        preview.structureOccurrences.where(
          (item) => item.displayName == 'КИР 03.060',
        ),
        hasLength(2),
      );
    },
  ...
  test(
    'ambiguous parent is reported as conflict instead of auto-picking nearest row',
    () {
      final preview = builder.build(
        machineVersionId: 'ver-1',
        rows: const [
